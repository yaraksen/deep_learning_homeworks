...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        """
        optimizer.zero_grad()
        indices = indices[:, :lengths.max()].to(device) # (B, L)
        # lengths = lengths.to(device)

        logits = model(indices[:, :-1], lengths - 1) # (B, L, V)
...
def train(model: LanguageModel, optimizer: torch.optim.Optimizer, scheduler: Optional[Any],
          train_loader: DataLoader, val_loader: DataLoader, num_epochs: int, num_examples=5, model_name='rnn_bpe_2000'):
    """
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"model_{model_name}_{val_loss}.pth")
            print(f'saved better model with loss={val_loss}')
