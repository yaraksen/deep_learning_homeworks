...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        :return: array of shape (batch_size, num_features)
        """
        dx_hat = grad_output * (self.weight if self.affine else np.ones_like(grad_output))
        if self.training:
            B = grad_output.shape[0]
            return (1 / B) * self.inv_sqrt_var * (B * dx_hat - np.sum(dx_hat, axis=0) -
                                                    self.norm_input * np.sum(dx_hat * self.norm_input, axis=0))
        else:
